...
public class ShoppingCartController
{
    ...
    public void addToCart(@PathVariable int productId, Principal principal) {
        try {
            ...
            if (shoppingCartDao.isProductAlreadyAdded(productId, userId)) {
                shoppingCartDao.incrementQuantity(productId, userId);
            } else {
                ShoppingCartItem item = new ShoppingCartItem();
                item.setProduct(productDao.getById(productId));
                item.setQuantity(1);
                shoppingCartDao.create(item, userId);
            }
    ...
    public void updateCartItem(@PathVariable int productId, @RequestBody ShoppingCartItem item, Principal principal) {
        try {
            ...
            shoppingCartDao.updateQuantity(item, productId, userId);
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Oops... our bad.");
        }
    }
